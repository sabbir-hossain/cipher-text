function draw_g(size = 16) {
  ...
  pointList["seven"] = { 
    x1: x_unit * (totalLineX - 1), y1: ((totalLineX - 1) * y_unit),
  ...
  pointList["eight"] = { 
    x1: x_unit * (totalLineX - 1), y1: ((totalLineX - 1) * y_unit),
    x2: x_unit * totalLineX, y2: y_unit * (totalLineX - 1),
    x3: x_unit * totalLineX, y3: y_unit
  };

  pointList["nine"] = { 
    x1: x_unit * (totalLineX - 1), y1: ((totalLineX - 1) * y_unit),
    x2: x_unit * (totalLineX - 1), y2: y_unit * 2,
    x3: x_unit * 2, y3: y_unit * 2
  };

  pointList["ten"] = { 
    x1: x_unit * (totalLineX - 1), y1: ((totalLineX - 1) * y_unit),
    x2: x_unit * 2, y2: y_unit * (totalLineX - 1),
    x3: x_unit * 2, y3: y_unit * 2
  };
