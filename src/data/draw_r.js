...
export default function R(size = 16) {
  const pointList = {};

  const max_y = size;
  const max_x = max_y *  config['R'].ratio / 10;
  ...
  pointList["eleven"] = { 
    ...
    x2: (x_unit + constX), y2: y_unit * (totalLineY/2),
    x3: x_unit, y3:  (y_unit * (totalLineY/2)) - constY
  };

  pointList["twelve"] = { 
    x1: x_unit * totalLineX, y1: 0,
    x2: (x_unit + constX), y2: y_unit * (totalLineY/2),
    x3: x_unit, y3:  (y_unit * (totalLineY/2)) - constY
  };
